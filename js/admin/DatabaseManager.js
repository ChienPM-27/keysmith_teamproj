 ...
 *   orders: [
 *     {
 *       idOrder: Number,          // Mã đơn hàng (chuỗi số, không ký tự đặc biệt)
...
class DatabaseManager {
  constructor(initialData = {}) {
    ...
    // determine nextOrderId by looking at both new `idOrder` and legacy `id` values
    let nextOrderId =
      this.data.orders.reduce(
        (m, x) => Math.max(m, Number(x.idOrder) || 0, Number(x.id) || 0),
        0
      ) + 1;
    this.data.orders = (this.data.orders || []).map((o) => {
      // migrate/ensure numeric idOrder
      if (
        typeof o.idOrder === "undefined" ||
        o.idOrder === null ||
        o.idOrder === ""
      ) {
        // if old `id` exists and looks numeric, use it as idOrder
        if (typeof o.id !== "undefined" && o.id !== null && o.id !== "") {
          const p = Number(o.id);
          if (!Number.isNaN(p)) {
            o.idOrder = p;
          } else {
            o.idOrder = nextOrderId++;
          }
        } else {
          o.idOrder = nextOrderId++;
        }
      } else {
        const p = Number(o.idOrder);
        if (!Number.isNaN(p)) o.idOrder = p;
      }
      // keep old `id` for backward compatibility (per-table only)
      o.id = o.idOrder;

      if (typeof o.totalPrice === "undefined" && typeof o.total !== "undefined")
        o.totalPrice = o.total;
      if (!o.orderNumber) o.orderNumber = "o_" + (o.idOrder || Date.now());
      return o;
    });

    // ImportOrders: make sure we have a numeric receipt id in `id` and keep product reference in `productId`
    // assign idImportOrders sequentially; preserve `id` as product id when present
    let nextImportId =
      this.data.importOrders.reduce(
        (m, x) => Math.max(m, Number(x.idImportOrders) || 0),
        0
      ) + 1;
    this.data.importOrders = (this.data.importOrders || []).map((imp) => {
      // if earlier shape used `id` as product reference, keep it in productId
      const prod = this.getById("products", imp.id);
      if (prod) {
        imp.productId = imp.id;
      }

      // ensure numeric idImportOrders
      if (
        typeof imp.idImportOrders === "undefined" ||
        imp.idImportOrders === null ||
        imp.idImportOrders === ""
      ) {
        imp.idImportOrders = nextImportId++;
      } else {
        const p = Number(imp.idImportOrders);
        if (!Number.isNaN(p)) imp.idImportOrders = p;
        else imp.idImportOrders = nextImportId++;
      }
      return imp;
    });
  ...
  #handleNewOrder(order) {
    // normalize items and compute amountPrice
    order.items = (order.items || []).map((item) => {
      const amountPrice = (item.quantity || 0) * (item.unitPrice || 0);
      return { ...item, amountPrice };
    });

    // compute totalPrice and keep backward-compatible `total` field
    ...
    // assign a numeric order idOrder if not present (incremental)
    ...
    // keep legacy `id` for backwards compatibility within orders table
    order.id = order.idOrder;

    // ensure orderNumber exists (string code) for display/search
  ...
  #handleNewImportOrder(importOrder) {
    ...
    // - If caller provided `productId`, use it as the product reference.
    // - If not, and `id` was provided (old shape), treat that as the product id.
    ...
    if (typeof importOrder.productId !== "undefined") {
      ...
      // earlier shape used id as product id
      productIdRef = importOrder.id;
    }

    // assign a numeric idImportOrders (receipt id) if not present
    ...
    // keep explicit productId field for product reference
    if (productIdRef) importOrder.productId = productIdRef;

    // update product stock if we have a productId (or id as fallback)
